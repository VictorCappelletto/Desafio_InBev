...
class DataQualityEngine:
    ...
    def run_checks(self, data: List[Dict[str, Any]]) -> DataQualityReport:
        ...
        self.logger.info(f"Running {len(self.checks)} quality checks on {len(data)} records")
        ...
        for check in self.checks:
            try:
                ...
                self.logger.error(f"Check failed: {check.dimension.value}", exc_info=True)
        ...
        weighted_sum = sum(
            ...
            for result, check in zip(results, self.checks)
        )
