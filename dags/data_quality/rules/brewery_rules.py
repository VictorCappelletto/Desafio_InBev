...
VALID_BREWERY_TYPES = {
    ...
    'planning', 'contract', 'proprietor', 'closed'
}
...
def create_brewery_quality_engine(
    ...
    engine.add_check(CompletenessCheck(
    ...
    engine.add_check(AccuracyCheck(
        field_validators={
    ...
    engine.add_check(AccuracyCheck(
        field_validators={
            'state': lambda x: x in VALID_US_STATES if x else True,  # Optional but must be valid
        },
        threshold=0.95,
        weight=1.0
    ))
    ...
    business_rules = [
        ...
        lambda r: (r.get('latitude') and r.get('longitude')) or 
                 (not r.get('latitude') and not r.get('longitude')),
    ...
    engine.add_check(ConsistencyCheck(
    ...
    engine.add_check(ValidityCheck(
        schema=schema,
        threshold=0.95,
        weight=1.0
    ))
    ...
    engine.add_check(UniquenessCheck(
        key_fields='id',
