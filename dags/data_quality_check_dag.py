    ...
    try:
        ...
        with pyodbc.connect(sql_config.connection_string) as conn:
            ...
            for column in critical_columns:
                cursor.execute(
                    f"SELECT COUNT(*) FROM Breweries WHERE {column} IS NULL"
                )
...
def generate_quality_report(**context: Any) -> None:
    ...
    try:
        ...
        row_count_result = ti.xcom_pull(task_ids='check_row_count', key='row_count_check')
        null_result = ti.xcom_pull(task_ids='check_null_values', key='null_check')
        duplicate_result = ti.xcom_pull(task_ids='check_duplicates', key='duplicate_check')
        freshness_result = ti.xcom_pull(task_ids='check_data_freshness', key='freshness_check')
        ...
        report = {
            ...
            "summary": {
                "total_checks": 4,
                "passed": sum(1 for r in [row_count_result, null_result, duplicate_result, freshness_result] if r['status'] == 'passed'),
                "failed": 0
            }
        }
